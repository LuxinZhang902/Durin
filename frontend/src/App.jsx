...
function App() {
  const [currentPage, setCurrentPage] = useState('personal-info') // 'personal-info', 'id-upload', 'liveness', 'transaction-upload', 'processing', 'decision', 'bankimport', 'bank', 'main', 'profile'
  ...
  const [activeTab, setActiveTab] = useState('fraud-detection') // 'fraud-detection', 'liveness-check', or 'user-protection'

  // New onboarding flow handlers
  const handlePersonalInfoComplete = (data) => {
    setUserData(data)
    setCurrentPage('id-upload')
  }

  const handleIDUploadComplete = (data) => {
    setIdData(data)
  ...
  const handleLivenessComplete = (data) => {
    ...
    setCurrentPage('transaction-upload')
  }

  const handleTransactionUploadComplete = (data) => {
    setTransactionData(data)
    setCurrentPage('processing')
  }

  const handleProcessingComplete = (decision) => {
...
Report generated by Durin Fraud Detection System
  ...
  if (currentPage === 'id-upload') {
    return <IDUpload onComplete={handleIDUploadComplete} onBack={() => setCurrentPage('personal-info')} userData={userData} />
  }

  if (currentPage === 'liveness') {
    return <LivenessCheck onComplete={handleLivenessComplete} onBack={() => setCurrentPage('id-upload')} userData={userData} />
  }

  if (currentPage === 'transaction-upload') {
    return <TransactionUpload onComplete={handleTransactionUploadComplete} onBack={() => setCurrentPage('liveness')} userData={userData} />
  }

  if (currentPage === 'processing') {
    return <ProcessingScreen onComplete={handleProcessingComplete} userData={userData} idData={idData} livenessData={livenessData} transactionData={transactionData} />
  }

  if (currentPage === 'decision') {
    return <CreditDecision decision={creditDecision} userData={userData} onGoToDashboard={handleGoToDashboard} />
  }

  // Old flow pages (keep for backward compatibility)
  ...
  return (
    <div className="min-h-screen bg-dark-bg text-white relative overflow-hidden">
      ...
      {/* Tab Switcher */}
      <div className="sticky top-0 z-40 bg-dark-surface/80 backdrop-blur-2xl border-b border-dark-border/30">
        <div className="max-w-[1400px] mx-auto px-8">
          <div className="flex space-x-1">
            ...
            >
              ...
              {activeTab === 'fraud-detection' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-accent-blue to-accent-purple"></div>
              )}
            </button>

            <button
              onClick={() => setActiveTab('liveness-check')}
              className={`px-6 py-4 font-semibold transition-all relative ${
                activeTab === 'liveness-check'
                  ? 'text-white'
                  : 'text-gray-500 hover:text-gray-300'
              }`}
            >
              ...
              {activeTab === 'liveness-check' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-accent-cyan to-accent-blue"></div>
              )}
            </button>
            ...
            >
              ...
              {activeTab === 'user-protection' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-accent-purple to-accent-pink"></div>
              )}
            </button>
          </div>
        </div>
      ...
      <footer className="mt-12 border-t border-dark-border">
        <div className="max-w-7xl mx-auto px-6 py-6 text-center text-sm text-gray-500">
          <p>Durin – Hackathon Demo • Built with React, FastAPI, NetworkX & OpenAI</p>
        </div>
      </footer>
